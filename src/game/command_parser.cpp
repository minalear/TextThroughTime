...
Command process_input(const TokenGroup &tokens) {
    ...
    } else if (contains(command.primary, equip_list)) {
        command.type = COMMAND_TYPES::EQUIP;
    } else if (contains(command.primary, unequip_list)) {
        command.type = COMMAND_TYPES::UNEQUIP;
